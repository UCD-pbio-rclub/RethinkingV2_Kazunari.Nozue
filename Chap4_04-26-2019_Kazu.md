...
```
##             mean       sd       5.5%      94.5%
## mu    154.606996 0.411993 153.948551 155.265440
## sigma   7.731301 0.291383   7.265615   8.196987
```
...
```
##                 mu        sigma
## mu    0.1697396059 0.0002180234
## sigma 0.0002180234 0.0849058154
```
...
```
##                mu       sigma
## mu    1.000000000 0.001816112
## sigma 0.001816112 1.000000000
```
...
```
##             mean        sd       5.5%      94.5%    histogram
## mu    154.608681 0.4104825 153.953229 155.273297      ▁▁▅▇▂▁▁
## sigma   7.726876 0.2893822   7.265517   8.191133 ▁▁▁▂▅▇▇▃▁▁▁▁
```
...
```r
data.weight<-data.frame(weight=c(46.95,43.72,64.78,32.59,54.63))
#,expected_height="",eightynine_percent_interval="")
xbar<-mean(data.weight$weight)
list.4H1 <- alist(
    height ~ dnorm( mu , sigma ),
    mu<- a + b*(weight-xbar),
        a ~ dnorm( 100 , 20 ) ,
...
m4H1<-quap(list.4H1 , data=data.weight) # error
```

```
## Error in quap(list.4H1, data = data.weight): Cannot find 'height'.
...
```
## Error in precis(m4H1): object 'm4H1' not found
```
...
```
## Error in extract.samples(m4H1): object 'm4H1' not found
```

```r
mu.link <- function(weight) post$a + post$b*weight
#weight.seq <- seq( from=25 , to=70 , by=1 )
mu <- sapply( data.weight$weight , mu.link )
data.weight$expected.height<-mu.mean <- apply( mu , 2 , mean )
```
...
```r
data.weight$HPDI.5.5<-mu.HPDI <- apply( mu , 2 , HPDI , prob=0.89 )[1,]
```

```
## Error in apply(mu, 2, HPDI, prob = 0.89): dim(X) must have a positive length
```

```r
data.weight$HPDI.94.5<-mu.HPDI <- apply( mu , 2 , HPDI , prob=0.89 )[2,]
```
